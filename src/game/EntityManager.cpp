...
wiicraft::EntityManager::EntityManager()
{
    core::IEventManager::Get()->AddListener(
        fastdelegate::MakeDelegate(this, &EntityManager::OnPlayerEntitySpawn), EventDataSpawnPlayerEntity::EventType);
    core::IEventManager::Get()->AddListener(
        fastdelegate::MakeDelegate(this, &EntityManager::OnItemEntitySpawn), EventDataSpawnItemEntity::EventType);
    core::IEventManager::Get()->AddListener(
        fastdelegate::MakeDelegate(this, &EntityManager::OnEntityRemove), EventDataDestroyEntity::EventType);
    ...
    core::IEventManager::Get()->AddListener(
        fastdelegate::MakeDelegate(this, &EntityManager::OnEntityRelativeMove), EventDataEntityRelativeMove::EventType);
}
...
wiicraft::EntityManager::~EntityManager()
{
    ...
    core::IEventManager::Get()->RemoveListener(
        fastdelegate::MakeDelegate(this, &EntityManager::OnEntityRemove), EventDataDestroyEntity::EventType);
    core::IEventManager::Get()->RemoveListener(
        fastdelegate::MakeDelegate(this, &EntityManager::OnEntityMove), EventDataEntityMove::EventType);
    core::IEventManager::Get()->RemoveListener(
        fastdelegate::MakeDelegate(this, &EntityManager::OnEntityRelativeMove), EventDataEntityRelativeMove::EventType);
}
...
void wiicraft::EntityManager::Render(renderer::Renderer& renderer)
{
    ...
    {
        const math::Vector3f entityAABBPos = { static_cast<float>(entity.second->GetX() / 32.0f),
                                               static_cast<float>(entity.second->GetY() / 32.0f),
                                               static_cast<float>(entity.second->GetZ() / 32.0f) };
...
void wiicraft::EntityManager::OnItemEntitySpawn(core::IEventDataPtr eventData)
{
    ...
    ASSERT(itemEntity);
    mItemEntities[itemEntity->GetEntityID()] = std::make_shared<ItemEntity>(
...
void wiicraft::EntityManager::OnEntityRelativeMove(core::IEventDataPtr eventData)
{
    ...
    if (playerFindIt != mPlayerEntities.end())
    {
        playerFindIt->second->UpdatePositionAndRotation(
            entityRelativeMove->GetX(),
            entityRelativeMove->GetY(),
            entityRelativeMove->GetZ(),
            entityRelativeMove->GetYaw(),
            entityRelativeMove->GetPitch());
        return;
    }
    auto itemFindIt = mItemEntities.find(entityRelativeMove->GetEntityID());
    if (itemFindIt != mItemEntities.end())
    {
        itemFindIt->second->UpdatePositionAndRotation(
            entityRelativeMove->GetX(),
            entityRelativeMove->GetY(),
            entityRelativeMove->GetZ(),
            entityRelativeMove->GetYaw(),
